  ...
  return (
    <section id="education" className="py-24 scroll-mt-24">
      ...
      <div className="relative max-w-4xl mx-auto px-6">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 w-0.5 h-full bg-slate-200 dark:bg-slate-800 hidden sm:block"></div>
        
        <div className="space-y-12 sm:space-y-0">
          {data.map((edu, index) => (
            <div 
              key={edu.degree.Anglais} /* Uses English string as unique key */
              className={`relative flex flex-col sm:flex-row items-center sm:mb-16 ${index % 2 === 0 ? 'sm:flex-row-reverse' : ''}`}
            >
              <div className="hidden sm:block sm:w-1/2"></div>
              
              {/* Timeline Dot */}
              <div className="absolute left-1/2 -translate-x-1/2 z-10 bg-white dark:bg-slate-900 border-4 border-cyan-100 dark:border-cyan-900/30 p-2 rounded-full text-cyan-500 hidden sm:block">
                <div className="w-4 h-4 bg-cyan-500 rounded-full shadow-[0_0_15px_rgba(6,182,212,0.5)]"></div>
              </div>
              
              {/* Content Card */}
              <div className={`w-full sm:w-1/2 ${index % 2 === 0 ? 'sm:pr-12 sm:text-right' : 'sm:pl-12 text-left'}`}>
